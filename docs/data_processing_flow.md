...
- **ロジック**:
    ...
    - **ペアの取り出し (時間ベース・プレロール機構)**:
        - 新たなフレーム供給（キュー0枚から1枚以上の増加）を検知すると、直ちに消費を開始するのではなく **3サイクルのコールバック（約100ミリ秒）の意図的な遅延（プレロール）** を設ける。
        - **通常アニメーション時**: この100ミリ秒の間にCEFから約6枚のフレームがキューに貯金される。消費開始後は、OSタイマーのジッターによってCEFの生成が一瞬遅れた際も、この貯金から安定して2枚ずつフレームを引き出せるため、不規則なカクつき（微小ジッター）が完全に吸収される。
        - **静止画（カット出し）時**: CEFから1枚のフレームのみが供給され後続が来ない場合も、100ミリ秒の遅延後にその1枚が取り出され、TopとBottomの両フィールドに複製されて静止画として正しく出力される。
...
2.  **バックグラウンド抑制の無効化**:
    ...
    - **注意**: CEFの内部タイマー（60fps）を正確に維持するため、`--disable-frame-rate-limit` や `--disable-gpu-vsync` は使用しない。これらを使用すると、休眠からの復帰時に溜まった「タイマー負債」を取り戻そうと超高速でレンダリングが暴走し、激しいコマ飛び（ガタツキ）の原因となる。
    - 万が一、タイマーの遅れ等で突発的なフレームのバースト生成が発生した場合でも、GPU側のテクスチャキューのサイズを上限16枚に制限（超過分は即座に最古のフレームからドロップ）することで、アニメーションの強烈な遅延を防ぎ瞬時にリアルタイムへ追いつく自己防衛（セルフヒーリング）機構を備えている。
3.  **UIモニタリングとログ**:
