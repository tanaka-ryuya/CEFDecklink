			...
			if (displayModeIterator->Next(&deckLinkDisplayMode) == S_OK)
			{
				if (deckLinkDisplayMode != NULL)
				{
					...
					if (deckLinkDisplayMode->GetName(&modeNameBSTR) == S_OK)
					{
						...
						else if (m_deckLinkOutput->RowBytesForPixelFormat(bmdFormat8BitBGRA, m_frameWidth, &frameRowBytes) != S_OK)
						{
							fprintf(stderr, "Could not get frame row bytes\n");
						}
						else
						{
							// Get a BGRA frame 
							if (m_deckLinkOutput->CreateVideoFrame(m_frameWidth, m_frameHeight, frameRowBytes, bmdFormat8BitBGRA, bmdFrameFlagFlipVertical, &m_videoFrameGdi) != S_OK)
							{
...
void	GdiDraw (IDeckLinkVideoFrame* theFrame)
{
	...
	// query the video buffer for the video frame
	IDeckLinkVideoBuffer* videoBuffer = NULL;
	if (theFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&videoBuffer) != S_OK)
	{
		fprintf(stderr, "Could not query the video frame buffer\n");
		goto bail;
	}

	if (videoBuffer->StartAccess(bmdBufferAccessWrite) != S_OK)
	{
		fprintf(stderr, "Could not access DeckLink Video Buffer\n");
		goto bail;
	}
	...
	videoBuffer->GetBytes((void**)&pbDestData);
