...
// Video pixel format - fix to BGRA
static const std::pair<GUID, BMDPixelFormat> kVideoPixelFormat = std::make_pair(MFVideoFormat_RGB32, bmdFormat8BitBGRA);

// Map the supported color primaries from MFVideoPrimaries enumeration
static const std::map<MFVideoPrimaries, BMDColorspace> kSupportedColorspaces =
{
	std::make_pair(MFVideoPrimaries_BT709, bmdColorspaceRec709),
	std::make_pair(MFVideoPrimaries_EBU3213, bmdColorspaceRec601),
	std::make_pair(MFVideoPrimaries_SMPTE_C, bmdColorspaceRec601),
	std::make_pair(MFVideoPrimaries_BT2020, bmdColorspaceRec2020),
};
...
SourceReader::SourceReader() : 
	...
	m_colorPrimaries(MFVideoPrimaries_Unknown),
...
HRESULT SourceReader::ConfigureVideoDecoder()
{
	...
	hr = outputType->GetUINT32(MF_MT_VIDEO_PRIMARIES, &m_colorPrimaries);
	if (hr != S_OK)
		m_colorPrimaries = MFVideoPrimaries_Unknown;
	...
	if (hr != S_OK)
	{
		// Default stride is not stored, use the DeckLink calculation for frame width
	...
	// When the stride is negative, the frame is bottom up
	m_flipVertical = m_minimumStride < 0;
...
BMDColorspace SourceReader::GetVideoColorspace() const
{
	auto search = kSupportedColorspaces.find((MFVideoPrimaries)m_colorPrimaries);
	if (search != kSupportedColorspaces.end())
	{	
		return search->second;
	}
	else
	{
		// Unknown colorspace, use the default colorspace assigned when creating output frame.
...
{
	CComPtr<PlaybackVideoBuffer>	videoBuffer;
	bool							endOfStream = false;

	{
		...
		if (m_sourceReaderCondition.wait_for(lock, kSourceReaderTimeout, [&] { return !m_waitingOnVideoSample; }))
		{
			...
			if (m_readVideoSample.sample && (m_readVideoSample.sample.CopyTo(&readSample) == S_OK))
				videoBuffer.Attach(new PlaybackVideoBuffer(readSample, m_readVideoSample.timestamp, m_flipVertical));
	...
	if ((callback != nullptr) && !callback(videoBuffer, endOfStream))
		return false;
