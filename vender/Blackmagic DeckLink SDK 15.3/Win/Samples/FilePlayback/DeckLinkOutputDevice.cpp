...
DeckLinkOutputDevice::DeckLinkOutputDevice(CComPtr<IDeckLink> deckLink) :
	...
	m_videoColorspace(bmdColorspaceUnknown),
	m_frameDuration(0),
	m_frameTimescale(0),
...
bool DeckLinkOutputDevice::EnableOutput(BMDDisplayMode displayMode, BMDPixelFormat pixelFormat, long minimumStride, BMDColorspace videoColorspace, 
										BMDAudioSampleType audioSampleType, uint32_t audioChannelCount, IDeckLinkScreenPreviewCallback* screenPreviewCallback)
{
	...
	m_frameRowBytes = abs(minimumStride);

	m_pixelFormat = pixelFormat;
	m_videoColorspace = videoColorspace;
...
bool DeckLinkOutputDevice::DisplayPreviewFrame(CComPtr<PlaybackVideoBuffer>& videoBuffer, bool endOfStream)
{
	CComPtr<IDeckLinkVideoFrame>	outputVideoFrame;

	m_endOfStream = endOfStream;

	if (m_endOfStream || ! videoBuffer)
		return false;

	// Get stream time so we start playback from correct scheduled time
	m_scheduledStartTime = videoBuffer->GetStreamTime(m_frameTimescale);

	if (!GetOutputVideoFrame(videoBuffer, &outputVideoFrame))
		return false;
...
bool DeckLinkOutputDevice::ScheduleNextVideoFrame(CComPtr<PlaybackVideoBuffer>& videoBuffer, bool endOfStream)
{
	...
	if (endOfStream || (videoBuffer == nullptr))
		return false;

	streamTime = videoBuffer->GetStreamTime(m_frameTimescale);
...
bool DeckLinkOutputDevice::GetOutputVideoFrame(CComPtr<PlaybackVideoBuffer>& videoBuffer, IDeckLinkVideoFrame** outputVideoFrame)
{
	CComPtr<IDeckLinkMutableVideoFrame> mutableVideoFrame = nullptr;

	if (*outputVideoFrame != nullptr)
		return false;
	
	BMDFrameFlags frameFlags = videoBuffer->GetFrameFlags();

	// Create new video frame based on the provided buffer
	CComQIPtr<IDeckLinkVideoBuffer> deckLinkVideoBuffer = videoBuffer;
	if (m_deckLinkOutput->CreateVideoFrameWithBuffer(m_frameWidth, m_frameHeight, m_frameRowBytes, m_pixelFormat, frameFlags, deckLinkVideoBuffer, &mutableVideoFrame) != S_OK)
		return false;

	if (m_videoColorspace != bmdColorspaceUnknown)
	{
		// Set the frame colorspace
		CComQIPtr<IDeckLinkVideoFrameMutableMetadataExtensions> mutableMetadataExtensions = mutableVideoFrame;

		if (mutableMetadataExtensions->SetInt(bmdDeckLinkFrameMetadataColorspace, m_videoColorspace) != S_OK)
			return false;
	}

	CComQIPtr<IDeckLinkVideoFrame> videoFrame = mutableVideoFrame;

	if (m_outputWithConversion)
	{
		CComPtr<IDeckLinkVideoFrame> convertedVideoFrame;
		if (m_deckLinkFrameConverter->ConvertNewFrame(videoFrame, kConvertedPixelFormat, m_videoColorspace, nullptr, &convertedVideoFrame) != S_OK)
			return false;

		*outputVideoFrame = convertedVideoFrame.Detach();
	}
	else
	{
		*outputVideoFrame = videoFrame.Detach();
	}
