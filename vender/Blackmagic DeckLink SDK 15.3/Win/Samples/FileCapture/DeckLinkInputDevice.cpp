...
#include "VideoBufferAllocatorPool.h"
...
bool DeckLinkInputDevice::StartCapture(BMDDisplayMode displayMode, BMDAudioSampleType audioSampleType, uint32_t audioChannelCount, IDeckLinkScreenPreviewCallback* screenPreviewCallback, bool applyDetectedInputMode)
{
	...
	// Enable the video input with custom allocators, so a video frame captures directly into IMFMediaBuffer
	CComPtr<IDeckLinkVideoBufferAllocatorProvider> allocatorPool;
	allocatorPool.Attach(new VideoBufferAllocatorPool());
	if (m_deckLinkInput->EnableVideoInputWithAllocatorProvider(displayMode, m_pixelFormat, videoInputFlags, allocatorPool) != S_OK)
		return false;
...
{	
	...
	if ((notificationEvents & bmdVideoInputDisplayModeChanged) || (pixelFormat != m_pixelFormat))
	{
		...
		// Re-enable the video input with the new mode/format
		// Use custom allocators, so a video frame captures directly into IMFMediaBuffer
		CComPtr<IDeckLinkVideoBufferAllocatorProvider> allocatorPool;
		allocatorPool.Attach(new VideoBufferAllocatorPool());
		if (m_deckLinkInput->EnableVideoInputWithAllocatorProvider(newMode->GetDisplayMode(), pixelFormat, bmdVideoInputEnableFormatDetection, allocatorPool) != S_OK)
		{
