...
void BMDOpenGLOutput::SetPreroll()
{
	...
	{
		int frameRowBytes;
		if (pDLOutput->RowBytesForPixelFormat(bmdFormat8BitBGRA, uiFrameWidth, &frameRowBytes) != S_OK)
			goto bail;

		// Flip frame vertical, because OpenGL rendering starts from left bottom corner
		if (pDLOutput->CreateVideoFrame(uiFrameWidth, uiFrameHeight, frameRowBytes, bmdFormat8BitBGRA, bmdFrameFlagFlipVertical, &pDLVideoFrame) != S_OK)
			goto bail;
...
void BMDOpenGLOutput::RenderToDevice(IDeckLinkVideoFrame* pDLVideoFrame)
{
	EnterCriticalSection(&pMutex);
	void*	pFrame;
	IDeckLinkVideoBuffer* pDLVideoBuffer = NULL;

	if (pDLVideoFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&pDLVideoBuffer) != S_OK)
	{
		LeaveCriticalSection(&pMutex);
		return;
	}
	...
	if (pDLVideoBuffer->StartAccess(bmdBufferAccessWrite) == S_OK)
	{
		if (pDLVideoBuffer->GetBytes((void**)&pFrame) == S_OK)
			glReadPixels(0, 0, uiFrameWidth, uiFrameHeight, GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV, pFrame);

		pDLVideoBuffer->EndAccess(bmdBufferAccessWrite);
	}

	pDLVideoBuffer->Release();
