...
HRESULT ImageWriter::WriteBgra32VideoFrameToPNG(IDeckLinkVideoFrame* bgra32VideoFrame, const std::string& pngFilename)
{
	...
	IDeckLinkVideoBuffer*				bgra32VideoFrameBuffer = NULL;
	...
	result = bgra32VideoFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&bgra32VideoFrameBuffer);
	if (FAILED(result))
		goto bail;

	result = bgra32VideoFrameBuffer->StartAccess(bmdBufferAccessRead);
	if (FAILED(result))
		goto bail;

	bufferAccessed = true;

	bgra32VideoFrameBuffer->GetBytes(&bgra32FrameBytes);
	if (bgra32FrameBytes == NULL)
	{
		fprintf(stderr, "Could not get DeckLinkVideoBuffer buffer pointer\n");
...
bail:
	...
	if (bgra32VideoFrameBuffer != NULL)
