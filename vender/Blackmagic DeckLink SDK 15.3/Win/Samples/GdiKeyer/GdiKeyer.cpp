...
int	OutputGraphic (IDeckLinkDisplayMode* deckLinkDisplayMode)
{
	...
	if (deckLinkDisplayMode != NULL)
	{
		...
		if (deckLinkDisplayMode->GetName(&modeNameBSTR) == S_OK)
		{
			...
			else if (deckLinkOutput->RowBytesForPixelFormat(bmdFormat8BitARGB, m_frameWidth, &frameRowBytes) != S_OK)
			{
			...
			else
			{
				...
				if (deckLinkOutput->CreateVideoFrame(m_frameWidth, m_frameHeight, frameRowBytes, bmdFormat8BitARGB, bmdFrameFlagFlipVertical, &m_videoFrameGdi) != S_OK)
				{
...
void	GdiDraw (IDeckLinkVideoFrame* theFrame)
{
	...
	IDeckLinkVideoBuffer* videoBuffer = NULL;							// query the video buffer
	...
	videoBuffer->GetBytes((void**)&pbDestData);							// get frame buffer pointer
