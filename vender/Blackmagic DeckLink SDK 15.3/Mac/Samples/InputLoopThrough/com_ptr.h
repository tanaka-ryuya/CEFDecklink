...
public:
	...
	template<typename U>
	bool operator==(const com_ptr<U>& other) const;
...
template<typename T>
template<typename U>
bool com_ptr<T>::operator==(const com_ptr<U>& other) const
{
	if (!m_ptr && other == nullptr)
		return true;
	
	if (!m_ptr || other == nullptr)
		return false;

		auto IID_IUnknown = CFUUIDGetUUIDBytes(IUnknownUUID);
	com_ptr<IUnknown> thisIUnknown(IID_IUnknown, com_ptr<T>(m_ptr));
	com_ptr<IUnknown> otherIUnknown(IID_IUnknown, other);

	return thisIUnknown == otherIUnknown;
}
