...
// Frame parameters
int32_t          gFrameRowBytes = 0;
BMDTimeValue     gFrameDuration = 0;
BMDTimeScale     gTimeScale = 0;
int32_t          gFrameWidth = 0;
int32_t          gFrameHeight = 0;
...
static void FillBlue(IDeckLinkMutableVideoFrame* theFrame)
{
	...
	if (videoBuffer->GetBytes((void**)&nextWord) != S_OK)
		goto bail;
...
public:
	...
	// IDeckLinkAncillaryPacket
	BMDAncillaryDataSpace STDMETHODCALLTYPE GetDataSpace() override
	{
...
HRESULT ScheduleNextFrame(CEA708::Encoder& CC708Encoder, IDeckLinkOutput* deckLinkOutput, IDeckLinkVideoFrame* videoFrame)
{
	...
	unsigned fps = (unsigned)gTimeScale / gFrameDuration;
	...
	if (CC708Encoder.pop(&packet))
	{
		...
		ancillaryPacket = new CaptionAncillaryPacket(packet);

		result = frameAncillaryPackets->AttachPacket(ancillaryPacket);

		// The attachment will hold reference to the ancillary packet, so we can release here.
		ancillaryPacket->Release();
	...
	result = deckLinkOutput->ScheduleVideoFrame(videoFrame, gTotalFramesScheduled*gFrameDuration, gFrameDuration, gTimeScale);
	if (result != S_OK)
	{
...
int main()
{
	...
	CEA708::Encoder*        CC708Encoder     = NULL;
	HRESULT                 result;
	...
	result = deckLinkDisplayMode->GetFrameRate(&gFrameDuration, &gTimeScale);
	if (result != S_OK)
	{
	...
	CC708Encoder = new CEA708::Encoder(gFrameDuration, gTimeScale);
...
bail:
	...
	// Release the DeckLink display mode interface
