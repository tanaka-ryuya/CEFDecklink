...
{
	bool		result = false;
	long		numWords = m_height * m_rowBytes / 4; // number of words in the frame
	...
	if (m_videoFrames[index]->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&videoFrameBuffer) != S_OK)
	{
		printf("Could not query the video frame buffer\n");
		goto bail;
	}
	
	if (videoFrameBuffer->StartAccess(bmdBufferAccessWrite) != S_OK)
	{
		printf("Could not access the video frame buffer for writing\n");
		goto bail;
	}
	
	bufferAccessed = true;
	
	if (videoFrameBuffer->GetBytes((void **)&nextWord) != S_OK)
	{
...
bool	ETTHelper::createFrames()
{
	...
	{
		if (m_deckLinkOutput->CreateVideoFrame((int32_t)m_width, (int32_t)m_height, (int32_t)m_rowBytes, PIXEL_FMT, bmdFrameFlagDefault, &m_videoFrames[i]) != S_OK)
		{
...
bool	ETTHelper::setupDeckLinkOutput()
{
    ...
    while (displayModeIterator->Next(&deckLinkDisplayMode) == S_OK)
    {
		if (deckLinkDisplayMode->GetDisplayMode() == BMD_DISPLAYMODE)
		{
			...
			int32_t rowBytes;
			m_deckLinkOutput->RowBytesForPixelFormat(PIXEL_FMT, (int32_t)m_width, &rowBytes);
			m_rowBytes = rowBytes;
...
ETTHelper::ETTHelper(IDeckLink *deckLink)
	...
	, m_width(-1), m_height(-1), m_rowBytes(-1), m_timeScale(0), m_frameDuration(0)
{
