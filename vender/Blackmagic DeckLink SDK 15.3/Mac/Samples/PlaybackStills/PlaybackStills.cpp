...
void PlaybackStills(IDeckLinkOutput* deckLinkOutput, IDeckLinkVideoFrame* playbackFrame, std::set<std::string>& pngFiles, long updateIntervalms, bool loopPlayback, bool convertOutput)
{
	...
	auto pngFileIter = pngFiles.begin();
	...
	if (convertOutput)
	{
		...
		if (deckLinkOutput->RowBytesForPixelFormat(kConvertedPixelFormat, (int32_t)playbackFrame->GetWidth(), &outputBytesPerRow) != S_OK)
		{
			fprintf(stderr, "Could not get row bytes for pixel format\n");
			goto bail;
		}

		if (deckLinkOutput->CreateVideoFrame((int32_t)playbackFrame->GetWidth(), (int32_t)playbackFrame->GetHeight(), outputBytesPerRow,
	...
	{
		result = ImageLoader::ConvertPNGToDeckLinkVideoFrame(*pngFileIter, playbackFrame);
		if (result != S_OK)
		{
		...
		{
			// Timeout
			if (++pngFileIter == pngFiles.end())
			{
				if (loopPlayback)
					pngFileIter = pngFiles.begin();
				else
					playbackRunning = false;
			}
		}
...
int main(int argc, char* argv[])
{
	...
	if (selectedDeckLinkOutput->RowBytesForPixelFormat(ImageLoader::kImageLoaderPixelFormat, (int32_t)displayModes[displayModeIndex]->GetWidth(), &frameRowBytes) != S_OK)
	{
		fprintf(stderr, "Could not get row bytes for pixel format\n");
		goto bail;
	}
