...
HRESULT ImageWriter::WriteBgra32VideoFrameToPNG(IDeckLinkVideoFrame* bgra32VideoFrame, const std::string& pngFilename)
{
	...
	IDeckLinkVideoBuffer* bgra32VideoFrameBuffer = nullptr;
	...
	if (bgra32VideoFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&bgra32VideoFrameBuffer) != S_OK)
	{
		fprintf(stderr, "Could not query DeckLink Video Buffer\n");
		goto bail;
	}

	if (bgra32VideoFrameBuffer->StartAccess(bmdBufferAccessRead))
	{
	...
	if (bgra32VideoFrameBuffer->GetBytes((void**)&deckLinkBuffer) != S_OK)
	{
...
bail:
	if (bufferAccessed)
		bgra32VideoFrameBuffer->EndAccess(bmdBufferAccessRead);

	if (bgra32VideoFrameBuffer != nullptr)
		bgra32VideoFrameBuffer->Release();
