...
void BMDOpenGLOutput::SetPreroll()
{
	...
	constexpr BMDPixelFormat kOutputPixelFormat = bmdFormat8BitBGRA;

	if (pDLOutput->RowBytesForPixelFormat(kOutputPixelFormat, uiFrameWidth, &frameRowBytes) != S_OK)
		goto bail;
	...
	for (uint32_t i=0; i < 3; i++)
	{
		...
		if (pDLOutput->CreateVideoFrame(uiFrameWidth, uiFrameHeight, frameRowBytes, kOutputPixelFormat, bmdFrameFlagFlipVertical, &pDLVideoFrame) != S_OK)
			goto bail;
...
{
	void*	pFrame;
	IDeckLinkVideoBuffer* pDLVideoFrameBuffer = NULL;
	
	if (pDLVideoFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&pDLVideoFrameBuffer) != S_OK)
		return;
	
	if (pDLVideoFrameBuffer->StartAccess(bmdBufferAccessWrite) != S_OK)
	{
		pDLVideoFrameBuffer->Release();
		return;
	}

	pDLVideoFrameBuffer->GetBytes((void**)&pFrame);

	Mutex.lock();
	memcpy(pFrame, pFrameBuf, pDLVideoFrame->GetRowBytes() * uiFrameHeight);
	Mutex.unlock();

	pDLVideoFrameBuffer->EndAccess(bmdBufferAccessWrite);
	pDLVideoFrameBuffer->Release();
