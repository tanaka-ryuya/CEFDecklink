...
void SignalGenHDR::UpdateFrameMetadata(com_ptr<IDeckLinkMutableVideoFrame>& displayFrame)
{
...
com_ptr<IDeckLinkMutableVideoFrame> SignalGenHDR::CreateColorbarsFrame()
{
	...
	frameWidth = (int32_t)m_selectedDisplayMode->GetWidth();
	frameHeight = (int32_t)m_selectedDisplayMode->GetHeight();

	hr = m_selectedDeckLinkOutput->RowBytesForPixelFormat(m_selectedPixelFormat, frameWidth, &displayFrameBytesPerRow);
	if (hr != S_OK)
		return nullptr;
	...
	if (hr != S_OK)
		return nullptr;
	UpdateFrameMetadata(displayFrame);
