...
static const REFIID IID_PinnedMemoryAllocator = {0x88,0x6a,0xeb,0xd0,0x5f,0xe3,0x47,0x40,0x84,0x99,0xc8,0x07,0x7e,0xae,0x52,0xb1};
...
bool OpenGLComposite::InitDeckLink()
{
...

	{
		...
		// For large frames use a reduced allocator buffer cache size to avoid out-of-memory
		com_ptr<IDeckLinkVideoBufferAllocatorProvider> captureAllocator(new (std::nothrow) InputAllocatorPool(this, mFrameWidth < 1920 ? 2 : 1));

		if (mDLInput->EnableVideoInputWithAllocatorProvider(displayMode, bmdFormat8BitYUV, bmdVideoInputFlagDefault, captureAllocator.get()) != S_OK)
			goto error;
	}
	...
	mPlayoutAllocator = new PinnedMemoryAllocator(this, VideoFrameTransfer::GPUtoCPU, 1, outputFrameRowBytes * mFrameHeight);
	...
	for (int i = 0; i < 10; i++)
	{
		...
		com_ptr<IDeckLinkMutableVideoFrame> mutableFrame;
		com_ptr<IDeckLinkVideoBuffer> outputFrameBuffer;

		if (mPlayoutAllocator->AllocateVideoBuffer(outputFrameBuffer.releaseAndGetAddressOf()) != S_OK)
			goto error;

		if (mDLOutput->CreateVideoFrameWithBuffer(mFrameWidth, mFrameHeight, outputFrameRowBytes, bmdFormat8BitBGRA, bmdFrameFlagFlipVertical, outputFrameBuffer.get(), mutableFrame.releaseAndGetAddressOf()) != S_OK)
			goto error;

		{
			com_ptr<IDeckLinkVideoFrame> videoFrame(IID_IDeckLinkVideoFrame, mutableFrame);
			mDLOutputVideoFrameQueue.push_back(videoFrame);
		}
	}
	...
	connect(mPlayoutDelegate, SIGNAL(playoutFrameCompleted(com_ptr<IDeckLinkVideoFrame>, BMDOutputFrameCompletionResult)), this, SLOT(PlayoutNextFrame(com_ptr<IDeckLinkVideoFrame>, BMDOutputFrameCompletionResult)), Qt::QueuedConnection);
...
void OpenGLComposite::VideoFrameArrived(IDeckLinkVideoInputFrame* inputFrame, bool hasNoInputSource)
{
	...
	if (mFastTransferExtensionAvailable)
	{
		com_ptr<PinnedMemoryAllocator> allocator(IID_PinnedMemoryAllocator, inputFrameBuffer);
...
void OpenGLComposite::PlayoutNextFrame(com_ptr<IDeckLinkVideoFrame> completedFrame, BMDOutputFrameCompletionResult completionResult)
{
	mMutex.lock();
	...
	auto outputVideoFrame = mDLOutputVideoFrameQueue.front();
	...
	HRESULT hr = mDLOutput->ScheduleVideoFrame(outputVideoFrame.get(), (mTotalPlayoutFrames * mFrameDuration), mFrameDuration, mFrameTimescale);
	if (SUCCEEDED(hr))
		mTotalPlayoutFrames++;
...
bool OpenGLComposite::Start()
{
	...
	for (unsigned i = 0; i < 5; i++)
	{
		...
		if (mDLOutput->ScheduleVideoFrame(outputVideoFrame.get(), (mTotalPlayoutFrames * mFrameDuration), mFrameDuration, mFrameTimescale) != S_OK)
			return false;
...
// built-in frame allocator.
//
// For this sample application a custom video buffer allocator is used to ensure each address
...
//
// The buffer cache delays the releasing of buffers until the cache fills up, thereby avoiding an
// allocate plus pin operation for every frame, followed by an unpin and deallocate on every frame.

PinnedMemoryAllocator::PinnedMemoryAllocator(QGLWidget* context, VideoFrameTransfer::Direction direction, unsigned cacheSize, unsigned bufferSize) :
	mContext(context),
	mRefCount(1),
...
bool PinnedMemoryAllocator::transferFrame(void* address, GLuint)
{
	...
	if (it == mFrameTransfer.end())
	{
		...
		it = mFrameTransfer.insert(std::make_pair(address, new VideoFrameTransfer(mBufferSize, address, mDirection))).first;
	}
...
HRESULT STDMETHODCALLTYPE	PinnedMemoryAllocator::QueryInterface(REFIID iid, LPVOID* ppv)
{
	...
	if (memcmp(&iid, &iunknown, sizeof(REFIID)) == 0 || memcmp(&iid, &IID_PinnedMemoryAllocator, sizeof(REFIID)) == 0)
	{
		*ppv = this;
	}
...
HRESULT STDMETHODCALLTYPE	PinnedMemoryAllocator::AllocateVideoBuffer (IDeckLinkVideoBuffer** allocatedBuffer)
{
	...
	if (mFrameCache.empty())
	{
		...
		if (posix_memalign(&memBuffer, 4096, mBufferSize) != 0)
...
InputAllocatorPool::InputAllocatorPool(QGLWidget* context, unsigned cacheSize) :
	mContext(context),
													...
													BMDPixelFormat, IDeckLinkVideoBufferAllocator **allocator)
{
	...
	com_ptr<PinnedMemoryAllocator> newAllocator;
	newAllocator.attach(new (std::nothrow) PinnedMemoryAllocator(mContext, VideoFrameTransfer::CPUtoGPU, mCacheSize, bufferSize));
	if (!newAllocator)
		return E_OUTOFMEMORY;
...
HRESULT STDMETHODCALLTYPE	DeckLinkVideoBuffer::QueryInterface(REFIID riid, void** ppvObject)
{
	...
	CFUUIDBytes iunknown = CFUUIDGetUUIDBytes(IUnknownUUID);
	if (memcmp(&riid, &iunknown, sizeof(REFIID)) == 0)
	{
		*ppvObject = this;
		AddRef();
	}
	else if (memcmp(&riid, &IID_IDeckLinkVideoBuffer, sizeof(REFIID)) == 0)
	{
		*ppvObject = static_cast<IDeckLinkVideoBuffer*>(this);
		AddRef();
	}
	else if (memcmp(&riid, &IID_PinnedMemoryAllocator, sizeof(REFIID)) == 0)
	{
...
HRESULT	PlayoutDelegate::ScheduledFrameCompleted (IDeckLinkVideoFrame* completedFrame, BMDOutputFrameCompletionResult result)
{
	...
	com_ptr<IDeckLinkVideoFrame> emitted(completedFrame); // AddRef completedFrame because our caller will release it
	emit playoutFrameCompleted(emitted, result);
	return S_OK;
}
