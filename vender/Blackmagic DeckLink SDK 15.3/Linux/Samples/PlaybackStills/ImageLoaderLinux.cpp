...
HRESULT ImageLoader::ConvertPNGToDeckLinkVideoFrame(const std::string& pngFilename, IDeckLinkVideoFrame* deckLinkVideoFrame)
{
	...
	IDeckLinkVideoBuffer* deckLinkVideoFrameBuffer = nullptr;
	...
	if (deckLinkVideoFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&deckLinkVideoFrameBuffer) != S_OK)
	{
		fprintf(stderr, "Could not query DeckLink Video Buffer\n");
		goto bail;
	}

	if (deckLinkVideoFrameBuffer->StartAccess(bmdBufferAccessWrite) != S_OK)
	{
		fprintf(stderr, "Could not lock DeckLink Video Buffer\n");
		goto bail;
	}

	bufferAccessed = true;

	if (deckLinkVideoFrameBuffer->GetBytes((void**)&deckLinkBuffer) != S_OK)
	{
...
bail:
	if (bufferAccessed)
		deckLinkVideoFrameBuffer->EndAccess(bmdBufferAccessWrite);

	if (deckLinkVideoFrameBuffer != nullptr)
		deckLinkVideoFrameBuffer->Release();
