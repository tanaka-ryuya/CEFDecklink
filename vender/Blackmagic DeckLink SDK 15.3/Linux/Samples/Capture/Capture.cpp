	...
	if (videoFrame)
	{
		...
		else
		{
			...
			if (g_videoOutputFile != -1)
			{
				...
				if (rightEyeFrame)
				{
					if (rightEyeFrame->QueryInterface(IID_IDeckLinkVideoBuffer, (void**)&videoFrameBuffer) == S_OK)
					{
						if (videoFrameBuffer->StartAccess(bmdBufferAccessRead) == S_OK)
						{
							videoFrameBuffer->GetBytes(&frameBytes);
							write(g_videoOutputFile, frameBytes, rightEyeFrame->GetRowBytes() * rightEyeFrame->GetHeight());
							videoFrameBuffer->EndAccess(bmdBufferAccessRead);
						}
						videoFrameBuffer->Release();
						videoFrameBuffer = NULL;
